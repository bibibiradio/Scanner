...
public class UrlJumpScanPlugin implements IScanPlugin {
    ...
    static private Logger logger = Logger.getLogger(UrlJumpScanPlugin.class);
    static private String[] payload;
    private String configPath;
    
    static{
        payload = new String[1];
        payload[0] = "http://xlxlxl/path";
    }
    ...
    private boolean check(ResponseData resData){
        String token = resData.getResponseHeader().get("Location");
        int resCode = resData.getStatusCode();
        if(resCode < 300 && resCode >= 400){
            return false;
        }
        
        if(token == null){
            token = resData.getResponseHeader().get("location");
            if(token == null){
                return false;
            }
        }
        
        if(token.equals("http://xlxlxl/path")){
            return true;
        }
